...
/**
 * Coverage of obsolete nodes in frontend
 */
...
class LayoutTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testLayoutFile()
    {
        ...
        $invoker(
            ...
            function ($layoutFile) {
                ...
                $this->assertSame(
                    [],
                    $layoutXml->xpath('/frontend//*[@output="toHtml"]'),
                ...
                foreach ($layoutXml as $handle) {
                    $this->assertNotContains(
                        (string)$handle['id'],
                        $this->_obsoleteNodes,
                        'This frontend handle is obsolete.'
                    );
                }
                ...
                if (false !== strpos(
                    ...
                    . '/view/adminhtml/frontend/sales_order'
                ) || false !== strpos(
                    $layoutFile,
                    $componentRegistrar->getPath(ComponentRegistrar::MODULE, 'Magento_Shipping')
                    . '/view/adminhtml/frontend/adminhtml_order'
                )
                ) {
                ...
                $this->assertSame(
                    [],
                    $layoutXml->xpath('/frontend//block[@class="Magento\Framework\View\Element\Text\ListText"]'),
                    'The class \Magento\Framework\View\Element\Text\ListTest' .
                    ' is not supposed to be used in frontend anymore.'
                );
            },
        ...
        $invoker(
            ...
            function ($layoutFile) {
                ...
                foreach ($layoutXml->xpath('//action[' . $methodFilter . ']') as $node) {
                    ...
                    $this->fail(
                        sprintf(
                            'Call of method "%s" via frontend instruction <action> is not allowed.',
                            $attributes['method']
                        )
                    );
                }
    ...
    /**
     * List of currently allowed (i.e. not refactored yet) methods for use in <action method="someMethod"/> frontend
     *  instruction.
     *
     * @SuppressWarnings(PHPMD.ExcessiveMethodLength)
     * Temporary method existing until <action> instruction in frontend is not eliminated, no need to split it.
     *
     * @return string[]
     */
